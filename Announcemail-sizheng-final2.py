...
def sent_email(mail_body):
    ...
    receiver = ['xujie@zju.edu.cn','jiahonglei@zju.edu.cn','ccx@zju.edu.cn','xujiyu@zju.edu.cn']
...
def get_content_xgb():
	...
	for alink in alinks:
		...
		if today_time.isoweekday()==1:
			for i in range(1,4):
				...
				if art_time==current_time:
					content_list.append('学工部通知：'+content+' 链接地址:'+main_url+art_url+' 时间：'+art_time)
		else:
		...
		#current_time=(datetime.date.today()-datetime.timedelta(days=1)).__format__('%Y-%m-%d')
			if art_time==current_time:
				content_list.append('学工部通知：'+content+' 链接地址:'+main_url+art_url)
	return content_list
	...
	for trlink in trlinks:
		...
		if today_time.isoweekday()==1:
			for i in range(1,4):
				current_time=(today_time-datetime.timedelta(days=i)).__format__('%Y-%m-%d')
				if art_time==current_time:
					content_list.append('研工部通知：'+content+' 链接地址：'+main_url+art_url+' 时间：'+art_time)
		else:
			current_time=(today_time-datetime.timedelta(days=1)).__format__('%Y-%m-%d')
			if art_time==current_time:
				content_list.append('研工部通知：'+content+' 链接地址：'+main_url+art_url)
	return content_list	
